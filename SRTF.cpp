...
using namespace std;
#include<iostream>
SRTF::SRTF(float comline_lambda, float comline_service_lambda){
...
void SRTF::handle_arrival(Event& E){
    clock = E.time;
    if(cpu_idle == 1){
    cpu_idle = 0;
...
void SRTF::handle_departure(Event &E){
    clock = E.time;
...
void SRTF::handle_sample(){
    avg_size_rdy_queue += ready_queue.size();
    samples_processed++;
}
int SRTF::get_num_proc_compl(){
    return num_of_processes_complete;
}
void SRTF::add_TTI(){
    TTI += (TWA - TWI);
}
Event SRTF:: get_event(){
    Event E = Utility::get_event(event_queue);
    return E;
}
void SRTF:: schedule_event( char requested_event_type, float event_time, PCB *pcb){
...
void SRTF::pushToRdyQueue(PCB* Pcb){
    ready_queue.push(Pcb);
}
void SRTF:: incrementPCBID(){
    PCBid++;
}
void SRTF:: add_proc_complete(){
  num_of_processes_complete++;
}

Result SRTF:: final_results(){
