...
using namespace std;
#include<iostream>
HRRN::HRRN(float comline_lambda, float comline_service_lambda){
...
void HRRN::handle_arrival(Event& E){
    clock = E.time;
    if(cpu_idle == 1){
    cpu_idle = 0;
...
void HRRN::handle_departure(Event &E){
    clock = E.time;
    ...
    HRRNRatioReevaluation(clock, ready_queue);//puts highest ratio in front
    PCB * next_in_line_pcb = ready_queue.front();
...
void HRRN::handle_sample(){
    avg_size_rdy_queue += ready_queue.size();
    samples_processed++;
}
int HRRN::get_num_proc_compl(){
    return num_of_processes_complete;
}
void HRRN::add_TTI(){
    TTI += (TWA - TWI);
}
Event HRRN:: get_event(){
    Event E = Utility::get_event(event_queue);
    return E;
}
void HRRN:: schedule_event( char requested_event_type, float event_time, PCB *pcb){
...
void HRRN::pushToRdyQueue(PCB* Pcb){
    ready_queue.push(Pcb);
}
void HRRN:: incrementPCBID(){
    PCBid++;
}
void HRRN:: add_proc_complete(){
  num_of_processes_complete++;
}

void HRRN:: HRRNRatioReevaluation(float clock, std::queue<PCB*> &ready_queue) {
    if (ready_queue.empty()) {
        return; 
    }
    std::queue<PCB*> temp_queue;
    PCB* highestPriorityProcess = ready_queue.front();
    float highestRatio = (clock - highestPriorityProcess->arrival_time + highestPriorityProcess->initial_service_time) / highestPriorityProcess->initial_service_time;

    ready_queue.pop(); 

    //unload and reload queue with highest ratio in the top slot
    while (!ready_queue.empty()) {
        PCB* currentProcess = ready_queue.front();
        float currentRatio = (clock - currentProcess->arrival_time + currentProcess->initial_service_time) / currentProcess->initial_service_time;
        if (currentRatio > highestRatio) {
            temp_queue.push(highestPriorityProcess);
            highestPriorityProcess = currentProcess;
            highestRatio = currentRatio;
        } else {
            temp_queue.push(currentProcess);
        }
        ready_queue.pop();
    }

    ready_queue.push(highestPriorityProcess); //push the process with the highest ratio to the front of the queue
